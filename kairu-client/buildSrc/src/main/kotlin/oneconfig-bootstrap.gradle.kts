...
afterEvaluate {
    ...
    // Compose/Skiko is flattened directly into the Kairu bootstrap JAR below.
    // Do not add the individual Compose artifacts to the normal Loom include
    // configuration or they would be packaged separately/nested.
...
base.archivesName.set("KairuSmpClient-${project.name}")

// Flatten the complete Compose bundle into the final Kairu bootstrap JAR.
// This intentionally uses zipTree rather than Loom's include(project(...)) so
// there is only one distributable JAR and the Compose classes/native resources
// are physically present in that same JAR.
val composeBundleJar = project(":modules:compose-bundle").tasks.named<Jar>("jar").flatMap { it.archiveFile }

fun configureComposeInto(task: TaskProvider<out AbstractArchiveTask>) {
    task.configure {
        dependsOn(composeBundleJar)
        from(composeBundleJar.map { zipTree(it) }) {
            // The standalone bundle's Fabric metadata is not needed inside the
            // Kairu mod; Kairu's own fabric.mod.json remains authoritative.
...
when {
    "remapJar" in tasks.names -> configureComposeInto(tasks.named<AbstractArchiveTask>("remapJar"))
    "jar" in tasks.names -> configureComposeInto(tasks.named<AbstractArchiveTask>("jar"))
}
