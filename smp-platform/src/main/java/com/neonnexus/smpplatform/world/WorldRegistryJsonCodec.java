...
public final class WorldRegistryJsonCodec {
    ...
    private static JsonObject resetObject(ResetPolicy policy) { JsonObject value = new JsonObject(); value.addProperty("kind", policy.kind()); if (policy instanceof ResetPolicy.Weekly weekly) { add(value,"day",weekly.day().name()); add(value,"time",weekly.time().toString()); add(value,"timezone",weekly.timezone().getId()); value.addProperty("safetyBackupRequired",weekly.safetyBackupRequired()); } else if (policy instanceof ResetPolicy.Interval interval) { value.addProperty("intervalSeconds", interval.interval().toSeconds()); value.addProperty("safetyBackupRequired", interval.safetyBackupRequired()); } else if (policy instanceof ResetPolicy.Manual manual) add(value,"reason",manual.reason()); return value; }
    private static ResetPolicy parseReset(JsonObject value) { return switch (string(value,"kind")) { case "none" -> new ResetPolicy.None(); case "manual" -> new ResetPolicy.Manual(string(value,"reason")); case "weekly" -> new ResetPolicy.Weekly(DayOfWeek.valueOf(string(value,"day")), LocalTime.parse(string(value,"time")), ZoneId.of(string(value,"timezone")), bool(value,"safetyBackupRequired")); case "interval" -> new ResetPolicy.Interval(java.time.Duration.ofSeconds(value.get("intervalSeconds").getAsLong()), bool(value,"safetyBackupRequired")); default -> throw new IllegalArgumentException("Unknown reset policy kind"); }; }
