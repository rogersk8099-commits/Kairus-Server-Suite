...
object ConfigRegistry {
    ...
    /**
     * Kairu fork change: the client is a control centre, not a browser for
     * every locally-installed mod.  The Compose shell therefore exposes only
     * Kairu's registered control tree.
     */
    private const val KAIRU_CONTROL_TREE_ID = "kairu_smp_client.json"

    val modCardConfigs: List<ConfigData>
        get() {
            val hudCards = hudModCardConfigs()
            val superseded = cardsSupersededByHudCards(hudCards)
            return configs.filter { it.id == KAIRU_CONTROL_TREE_ID && shouldShowModCard(it) && it !in superseded }
        }
