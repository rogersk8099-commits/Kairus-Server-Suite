...
public final class PointsService {
    ...
    public List<PointTransaction> history(Actor actor, UUID playerId, String currencyId, int limit, Instant before) { if (!actor.playerId().equals(playerId) && !actor.isPointsAdmin()) throw new Phase3Exception(Phase3Exception.Code.FORBIDDEN, "Cannot view another player's point history"); return transactions.required(() -> repository.history(new AccountKey(OwnerType.PLAYER, playerId, currencyId), limit(limit), before == null ? Instant.MAX : before)); }
