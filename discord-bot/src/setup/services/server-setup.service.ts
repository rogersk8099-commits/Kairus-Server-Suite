...
export interface GuildSetupAdapter {
  ...
  getBotAccessRoleId(): Promise<string | null>;
...
export class ServerSetupService {
  ...
  public async setup(guild: GuildSetupAdapter): Promise<ServerSetupResult> {
    const botRoleId = await this.assertBotPermissions(guild);
    ...
    for (const spec of this.blueprint.categories) {
      const result = await this.ensureCategory(guild, spec, roleIds, botRoleId);
      results.push(result.item);
      ...
      if (!parentId) {
        results.push(
          this.failed(
            ...
            spec.type,
            spec.name,
            `Parent category ${spec.parentKey} is unavailable.`,
          ),
        );
        continue;
      }
      results.push(
        await this.ensureChannel(guild, spec, parentId, roleIds, botRoleId),
      );
    }
  ...
  private async assertBotPermissions(
    ...
    if (missing.length > 0) {
      throw new SetupPreflightError(
        "The bot requires both Manage Roles and Manage Channels before server setup can start.",
      );
    }
    const highestPosition = await guild.getBotHighestRolePosition();
    if (highestPosition === null) {
      throw new SetupPreflightError(
        "The bot member could not be resolved in this guild; role hierarchy cannot be verified.",
      );
    }
    const botRoleId = await guild.getBotAccessRoleId();
    if (!botRoleId) {
      throw new SetupPreflightError(
        "The bot's managed Discord role could not be resolved; private-channel access cannot be reconciled safely.",
      );
    }
    return botRoleId;
  }
  ...
  private async ensureRole(
    ...
    try {
      ...
      if (!existing) {
        ...
        return {
          id: created.id,
          item: this.success(spec.resourceKey, "role", spec.name, "created"),
        };
      }
      ...
      const resolved = changed
        ? await guild.editRole(existing.id, spec)
        : existing;
  ...
  private async ensureCategory(
    ...
    try {
      const existing = await this.resolveChannel(
        guild,
        spec.resourceKey,
        "category",
        spec.name,
      );
      const changedDefinition = Boolean(
        existing && existing.name !== spec.name,
      );
      const category = existing
        ? changedDefinition
          ? await guild.editCategory(existing.id, spec)
          : existing
        : await guild.createCategory(spec);
      await this.persist(guild, spec.resourceKey, "category", category.id);

      const isolationChannel = asPolicyChannel(spec);
      const desired = createChannelPermissionPlan(
        guild.guildId,
        isolationChannel,
        roleIds,
        botRoleId,
      ...
      const changedPermissions = await guild.reconcilePermissionOverwrites(
        category.id,
        desired,
        controlledPolicyRoleIds(guild.guildId, roleIds, botRoleId),
      );
      return {
        id: category.id,
        item: this.success(
          ...
          spec.name,
          existing
            ? changedDefinition || changedPermissions
              ? "updated"
              : "reused"
            : "created",
        ),
  ...
  private async ensureChannel(
    ...
    try {
      ...
      const changedDefinition = existing
        ? !sameChannel(existing, spec, parentId)
        : false;
      const channel = existing
        ? changedDefinition
          ? await guild.editChannel(existing.id, spec, parentId)
          : existing
      ...
      const changedPermissions = await guild.reconcilePermissionOverwrites(
        channel.id,
        createChannelPermissionPlan(guild.guildId, spec, roleIds, botRoleId),
        controlledPolicyRoleIds(guild.guildId, roleIds, botRoleId),
      );
      return this.success(
        spec.resourceKey,
  ...
  private async resolveRole(
    ...
    const persisted = await this.resources.findByGuildAndKey(
      guild.guildId,
      spec.resourceKey,
    );
    const byId = persisted
      ? roles.find((role) => role.id === persisted.discordId)
      : undefined;
  ...
  private async resolveChannel(
    ...
    const byId = persisted
      ? channels.find((channel) => channel.id === persisted.discordId)
      : undefined;
  ...
  private async assertRoleEditable(
    ...
    if (highest === null || role.managed || role.position >= highest) {
      throw new SetupPreflightError(
        `The role ${expectedName} is managed or at/above the bot's highest role. Move the bot role above it before running setup.`,
      );
    }
  }
  ...
  private async persist(
    guild: GuildSetupAdapter,
    resourceKey: string,
    resourceType: SetupResourceKind,
    discordId: string,
  ): Promise<void> {
    await this.resources.upsert({
      guildId: guild.guildId,
      resourceKey,
      resourceType,
      discordId,
  ...
  private success(
    ...
    name: string,
    action: Exclude<SetupAction, "failed">,
  ): SetupItemResult {
  ...
  private failed(
    ...
    this.logger.warn("Discord server setup item failed", {
      key,
      kind,
      name,
      detail,
...
function uniqueExactName<T extends { name: string }>(
  items: readonly T[],
  name: string,
  kind: string,
): T | null {
  ...
  if (matches.length > 1)
    throw new Error(
      `Refusing to recover ${kind} ${name}: ${matches.length} exact-name matches are ambiguous.`,
    );
...
function sameRole(role: RoleSnapshot, spec: RoleBlueprint): boolean {
  return (
    role.name === spec.name &&
    role.color.toUpperCase() === spec.color.toUpperCase() &&
...
function sameChannel(
  ...
  if (spec.type === "text-channel")
    return channel.topic === spec.topic && channel.nsfw === spec.nsfw;
  return (
    channel.bitrate === spec.bitrate && channel.userLimit === spec.userLimit
  );
}
...
/** Every setup-owned access role is controlled, so audience changes cannot leave a stale staff/premium or bot grant. */
function controlledPolicyRoleIds(
  guildId: string,
  roleIds: ResolvedRoleIds,
  botRoleId: string,
): ReadonlySet<string> {
  return new Set(
    [guildId, botRoleId, ...STAFF_ROLE_KEYS, ...PREMIUM_ROLE_KEYS]
      .map((key) =>
        key === guildId || key === botRoleId ? key : roleIds[key as RoleKey],
      )
...
function countResults(items: readonly SetupItemResult[]): SetupCounts {
  return items.reduce<SetupCounts>(
...
export class DiscordJsGuildSetupAdapter implements GuildSetupAdapter {
  public constructor(
    ...
    private readonly reason = "Kairu declarative server setup",
  ) {}
  ...
  public async getBotAccessRoleId(): Promise<string | null> {
    const me = this.guild.members.me ?? (await this.guild.members.fetchMe());
    return me.roles.botRole?.id ?? null;
  }
  ...
  public async editRole(
    ...
    if (!role)
      throw new Error(`Role ${id} disappeared before it could be updated.`);
    return toRoleSnapshot(
      await role.edit({
        name: spec.name,
        color: spec.color,
  ...
  public async editCategory(
    ...
    return requireChannelSnapshot(
      await channel.edit({ name: spec.name, reason: this.reason }),
    );
  }
  ...
  public async editChannel(
    ...
    if (
      ...
      throw new Error(
        `Channel ${id} disappeared or changed type before update.`,
      );
    }
    if (spec.type === "text-channel") {
  ...
  public async reconcilePermissionOverwrites(
    ...
    if (
      ...
      throw new Error(
        `Channel ${channelId} cannot hold permission overwrites.`,
      );
    }
    const current = [...channel.permissionOverwrites.cache.values()].map(
      (overwrite) => ({
        id: overwrite.id,
        type: overwrite.type,
    ...
    const preserved = current.filter(
      (overwrite) => !controlledRoleIds.has(overwrite.id),
    );
    const merged = [
      ...preserved,
      ...desired.map((overwrite) => ({
        ...overwrite,
  ...
  private async fetchCategory(id: string): Promise<CategoryChannel> {
    ...
    if (!channel || channel.type !== ChannelType.GuildCategory)
      throw new Error(
        `Category ${id} disappeared or changed type before update.`,
      );
    return channel;
  }
}
...
function requireChannelSnapshot(
  channel: NonThreadGuildBasedChannel,
): ChannelSnapshot {
  const snapshot = toChannelSnapshot(channel)[0];
  if (!snapshot)
    throw new Error(
      `Discord returned an unexpected channel type (${channel.type}).`,
    );
  return snapshot;
}
