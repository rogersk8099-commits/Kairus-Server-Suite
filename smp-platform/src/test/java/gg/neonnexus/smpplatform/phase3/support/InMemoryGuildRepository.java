...
public final class InMemoryGuildRepository implements GuildRepository {
    ...
    public List<GuildInvite> findInvitesFor(UUID target,Instant now){return invites.values().stream().filter(i->i.targetPlayerId().equals(target)&&!i.isExpiredAt(now)).sorted(Comparator.comparing(GuildInvite::createdAt)).toList();}
