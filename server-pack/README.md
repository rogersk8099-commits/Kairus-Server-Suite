...
| SMPPlatform | **Required** for Neon Nexus world/guild/points functions | Suite-built `1.0.0` shaded JAR. | Owns the typed six-world registry, PostgreSQL/Flyway platform tables, durable outbox, and guild/points commands; packages staged lifecycle/event/admin modules. | Copy `SMPPlatform.jar` to `plugins/`; prefer environment database/API secrets, or use `plugins/SMPPlatform/database-password.txt` on MineKeep; keep destructive modules disabled until backup/world adapters pass staging. |
